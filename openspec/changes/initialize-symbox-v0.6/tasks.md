...
- [ ] 2.1 实现 Subject、Verb、Adj、tag、Worry、BindingRef、SVK 领域模型及输入不变量
- [ ] 2.2 实现 `physical`、`abstract`、`meta` 分类和 Verb domain/range 类型校验
- [ ] 2.3 实现 NodeKey codec 的转义、解析和 round-trip 测试
...
- [ ] 5.1 实现 create/delete 应用命令、唯一名称校验及删除后的事实撤回传播
- [ ] 5.2 实现项目相对 Python 源文件与限定函数名的安全加载和签名检查
- [ ] 5.3 实现 bind/unbind、Verb 标记、源码摘要持久化和加载时再验证
- [ ] 5.4 实现受控 mutation context，确保 check 异常或候选效果失败可回滚
- [ ] 5.5 添加重复对象、无效绑定、非 Verb 动词位、分类隔离和解除绑定测试
...
- [ ] 6.5 实现未配置/调用失败时精确名称降级及 degraded diagnostics
- [ ] 6.6 实现 `implies_tags` 自动派生、多来源撤回和同名显式 tag 保留
- [ ] 6.7 添加批量部分失败、阈值边界、降级、强制确认和 tag 来源测试

## 7. SVK now 关系断言

- [ ] 7.1 实现 `now` token parser，区分 Subject、Verb、位置论元和 `key=value` 论元
- [ ] 7.2 使用 Python signature 将 Subject 绑定为显式第一参数，并解析必需参数、默认值、重复参数和未知参数
- [ ] 7.3 实现 Verb True=通过/False=冲突的检查流程及结构化诊断
...
- [ ] 8.1 实现 Worry meta 对象、监控依赖注册及通用 bind/unbind 生命周期
- [ ] 8.2 实现属性候选更新时即时检查，并将 True/False 直接映射到健康节点
- [ ] 8.3 实现传播尾部对受影响 Worry 的固定点重评估
