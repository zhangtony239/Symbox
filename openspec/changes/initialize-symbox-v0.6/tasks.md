...
- [x] 8.4 实现状态签名、最大迭代边界和不收敛事务回滚诊断
- [x] 8.5 添加阈值跌破、恢复正常、间接值变化、解除绑定和循环测试

## 9. 查询接口

- [x] 9.1 实现 `list objects` 的确定排序和对象分类/Verb 摘要
- [x] 9.2 实现 `list verbs` 与绑定摘要过滤
- [x] 9.3 实现 `list <object>` 的 Adj/tag 来源、绑定、关系、真值与 justification 摘要
- [x] 9.4 实现未知对象错误及空集合成功语义
- [x] 9.5 添加查询只读性测试，验证重复查询不写磁盘、不执行有副作用 callable
...
- [x] 10.5 实现 `backup rollback` 的临时读取、完整状态验证和事务式原子恢复
- [x] 10.6 添加首次创建、跨项目隔离、删除含未知 id、损坏快照、并发 ref 与凭据排除测试
...
- [x] 11.3 验证每个写命令成功只提交一次、失败保持内存与磁盘状态不变
- [x] 11.4 更新 README，记录 v0.6 CLI、SVK 示例、Worry 极性、配置、状态目录和当前推理边界
- [x] 11.5 运行 formatter、linter、type checker、完整测试与 OpenSpec strict validation，并修复全部问题
